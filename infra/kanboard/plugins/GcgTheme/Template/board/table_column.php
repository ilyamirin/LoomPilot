...
<tr class="board-swimlane-columns-<?= $swimlane['id'] ?>">
    ...
    $gcg_column_classes = array('board-column-header', 'board-column-header-'.$column['id'], 'gcg-board-column');

    if (in_array($gcg_title, $gcg_terminal_titles, true)) {
        $gcg_column_classes[] = 'gcg-column-terminal';
    } elseif (in_array($gcg_title, $gcg_active_titles, true)) {
        $gcg_column_classes[] = 'gcg-column-active';
    } elseif (in_array($gcg_title, $gcg_entry_titles, true)) {
        $gcg_column_classes[] = 'gcg-column-entry';
    } else {
        $gcg_column_classes[] = 'gcg-column-neutral';
    }
    ?>
    <th class="<?= implode(' ', $gcg_column_classes) ?>" data-column-id="<?= $column['id'] ?>">
                ...
                <?php else: ?>
                    <span class="dropdown">
                        ...
                        <?php if ($column['nb_tasks'] > 0 && $this->user->hasProjectAccess('TaskModificationController', 'update', $column['project_id'])): ?>
                            <span class="dropdown">
                                <a href="#" class="dropdown-menu"><i class="fa fa-sort"></i></a>
                                <ul>
                                    <li>
